...
var GameHandler =
{
   ...
   /**
    * Webkit Audio API support
    */
   webkitAudioContext: null,
   webkitAudioComp: null,
   webkitAudioGain: null,
   sounds: {},
   ...
   init: function(minimumSize, padding)
   {
      ...
      // Chrome Webkit audio API detection
      this.webkitAudioContext = (typeof webkitAudioContext === "function" ? new webkitAudioContext() : null);
      if (this.webkitAudioContext)
      {
         this.webkitAudioGain = this.webkitAudioContext.createGainNode();
         this.webkitAudioGain.gain.value = 0.333;
         this.webkitAudioComp = this.webkitAudioContext.createDynamicsCompressor();
         this.webkitAudioGain.connect(this.webkitAudioComp);
         this.webkitAudioComp.connect(this.webkitAudioContext.destination);
      }
   },
   ...
   loadSound: function(url, id)
   {
      if (this.webkitAudioContext)
      {
         ...
         request.onload = function() {
            me.sounds[id] = me.webkitAudioContext.createBuffer(request.response, false);
         };
         request.send();
      }
   },
   ...
   {
      if (this.soundEnabled && this.webkitAudioContext && this.sounds[id])
      {
         var source = this.webkitAudioContext.createBufferSource();
         source.buffer = this.sounds[id];
         source.connect(this.webkitAudioGain);
         source.noteOn(0);
      }
   }
};
...
(function()
{
   Game.Main = function()
   {
      ...
      window.addEventListener("MozGamepadButtonDown", function(e) { buttonPressed(e, true); } );
      window.addEventListener("MozGamepadButtonUp", function(e) { buttonPressed(e, false); } );
      window.addEventListener("MozGamepadAxisMove", function(e) {
            if (me.sceneIndex !== -1)
            {
               me.scenes[me.sceneIndex].onAxisHandler(e.axis, e.value);
            }
         });
   };
   ...
   Game.Main.prototype =
   {
      ...
      frame: function frame()
      {
         ...
         if (GameHandler.webkitGamepad && this.sceneIndex !== -1)
         {
            for (var i=0,pad; i<navigator.webkitGamepads.length; i++)
            {
               if (pad = navigator.webkitGamepads[i])
               {
